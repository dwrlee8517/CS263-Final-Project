...
def generate_deepinfra_response(prompt: str|List[Dict], *, deepinfra_api_key=deepinfra_api_key, stream=False, **gen_params):
    ...
    # Some Gemini backends behind OpenAI-compatible APIs are strict about extra params.
    # Keep Gemini calls minimal to avoid provider-side incompatibility errors.
...
def generate_gemini_response(prompt: str | List[Dict], *, gemini_api_key=gemini_api_key, stream=False, **gen_params):
    ...
    model = gen_params.get("model", "gemini-3-flash-preview")
    reasoning_effort = gen_params.get("reasoning_effort")
    if reasoning_effort is None:
        # Optional compatibility: map existing 'reasoning' knob to Gemini-style effort.
...
def build_zero_shot_prompt(claim_text: str) -> str:
    return (
        "You are a fact-checking classifier.\n"
        "Classify the claim into exactly one label: true, false, or mixture.\n"
        "Definitions:\n"
        "- true: the claim is generally accurate.\n"
        "- false: the claim is generally inaccurate.\n"
        "- mixture: the claim is partially true / mostly true / mixed.\n\n"
        "Return only one word from this set: true, false, mixture.\n\n"
        f"Claim: {claim_text}\n"
        "Label:"
    )
...
def safe_model_label(prompt: str, *, retries: int = 1, **kwargs) -> tuple[str | None, str]:
    last_raw = ""
    for _ in range(retries + 1):
        raw = generate_model_response(prompt, **kwargs)
        last_raw = raw or ""
...
def run_zero_shot_eval(dataset_split, *, text_key: str, label_mapper, dataset_name: str, config: Dict):
    ...
    for ex in sampled:
        ...
        prompt = build_zero_shot_prompt(ex[text_key])
...
# Run a quick, small-sample zero-shot benchmark.
zero_shot_intel_rows = run_zero_shot_eval(
    intel_ds[ZERO_SHOT_CONFIG["split"]],
    text_key="text",
    label_mapper=lambda ex: INTEL_TO_EVAL_LABEL.get(ex["label_text"]),
    dataset_name="Intel sample",
    config=ZERO_SHOT_CONFIG,
)

zero_shot_pubhealth_rows = run_zero_shot_eval(
    source_ds[ZERO_SHOT_CONFIG["split"]],
    text_key="claim",
    label_mapper=lambda ex: PUBHEALTH_TO_EVAL_LABEL.get(ex["label"]),
    dataset_name="PubHealth sample",
    config=ZERO_SHOT_CONFIG,
)
# %%
